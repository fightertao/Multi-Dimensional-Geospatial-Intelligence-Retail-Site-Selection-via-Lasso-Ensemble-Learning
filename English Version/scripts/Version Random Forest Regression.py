...
# 4. Machine Learning Training (Random Forest)
features = ['k12', 'all_s', 'ind', 'hosp', 'mall', 'off', 'bus', 'prox']
X = df_grid[features]
y = df_grid['y_actual']
#  Feature Interaction (Retained for 36-dimensional feature space requirement)
...
# Test  R²: 0.5264
# Test MAE: 0.16
# Model generalization capability is within the acceptable range for Random Forest.
# Site Selection Capability (Top-K Coverage Analysis)
# Top 20% of grids covered 98.53% of existing shops.
# Prediction Strength: Exceptional(Highly significant spatial correlation).
...
# 4.4.Display Key Driving Factors
...
print("\n" + "=" * 60)
print("Top 10 Key Driving Factors (Random Forest)")
print("-" * 60)
for i, (idx, row) in enumerate(top_10_factors.iterrows(), 1):
    print(f"{i:2d}. {row['Feature']:25s} | Score: {row['Importance']:.4f}")
print("=" * 60)

# 5.Overfitting Diagnosis
if r2_train - r2_test > 0.2:
    print("WARNING: Significant overfitting detected (Training performance vastly exceeds Testing).")
else:
    print("Model generalization capability is within the acceptable range for Random Forest.")
# 6. Potential Calculation & Spatial Ranking
# Performing full-sample prediction for comprehensive visualization
df_grid['y_pred'] = rf_model.predict(X_poly)
df_grid['gap'] = df_grid['y_pred'] - df_grid['y_actual']
print("\n" + "=" * 60)
print("Site Selection Reliability Verification (Top-K Coverage Analysis)")
print("=" * 60)

df_sorted = df_grid.sort_values(by='y_pred', ascending=False)
top_k = int(len(df_sorted) * 0.2)
...
print(f"Top 20% of grids covered {coverage_rate * 100:.2f}% of existing shop locations.")
if coverage_rate >= 0.6: print("Predictive Strength: Exceptional")
print("=" * 60)
...
m.save("Map version Random Forest Regression.html")
