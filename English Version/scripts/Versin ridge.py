...
for ln in np.arange(LON_MIN, LON_MAX, STEP):
    for lt in np.arange(LAT_MIN, LAT_MAX, STEP):
        ...
        for c in CENTERS:
            ...
            # The smoothing term (0.01) prevents division by zero and moderates extreme values at the c
...
#4.Machine Learning Training (Ridge Regression)
...
#4.3. Data Normalization for Ridge Regression
scaler = StandardScaler()
X_poly_scaled = scaler.fit_transform(X_poly)

#4.4. Model Training: Automated Ridge Regularization
print("\n" + "="*60)
print("Running Automated Ridge Regularization Optimization...")
model = RidgeCV(alphas=[0.1, 1.0, 10.0, 100.0], cv=5)
model.fit(X_poly_scaled, y)

#4.5. Storing Predictions for Visualization and Spatial Analysis
df_grid['y_pred'] = model.predict(X_poly_scaled)
df_grid['gap'] = df_grid['y_pred'] - df_grid['y_actual']

print(f"Ridge Optimization Complete!")
print(f"Feature Status: Retained all {len(model.coef_)} features with smoothed weights.")
print("="*60)

#4.6. Ranking Top 10 Site Selection Drivers
print("\nTop Driving Factors Identified by Ridge Regression:")
important_feats = sorted(zip(feature_names, model.coef_), key=lambda x: abs(x[1]), reverse=True)
for name, val in important_feats[:10]:
    status = "[Positive Driver]" if val > 0 else "[Negative Suppressor]"
    print(f"Feature: {name:20} | Weight: {val:8.4f} | {status}")

# 5. Rigorous Model Validation Module
from sklearn.model_selection import train_test_split
from sklearn.metrics import r2_score, mean_absolute_error
print("\n" + "="*60)
print(" Generalization Assessment (Train/Test Split Validation)")
print("="*60)
# 5.1. Train / Test Partitioning
X_train, X_test, y_train, y_test = train_test_split(
    X_poly, y, test_size=0.3, random_state=42
)
#5.2. Re-training to Prevent Data Leakage
model_valid = LinearRegression()
model_valid.fit(X_train, y_train)
# 5.3. Prediction Execution
y_train_pred = model_valid.predict(X_train)
y_test_pred  = model_valid.predict(X_test)
# 5.4. Statistical Performance Metrics
...
# 5.5. Overfitting Diagnosis
if r2_train - r2_test > 0.15:
    print("⚠️ Potential Overfitting Detected (Significant variance between Train/Test performance)")
else:
    print("✅ Strong Model Generalization Capability")

# 6. Top-K Spatial Coverage Analysis (Core Site Selection Metric)
print("\n" + "="*60)
print(" Spatial Predictive Reliability (Top-K Coverage Analysis)")
print("=" * 60)

#6.1. Spatial Sorting based on Prediction Intensity
df_sorted = df_grid.sort_values(by='y_pred', ascending=False)
# 6.2. High-Potential Grid Extraction (Top 20%)
top_ratio = 0.2
top_k = int(len(df_sorted) * top_ratio)
df_top = df_sorted.head(top_k)
# 6.3. Recall Calculation (Site Coverage Rate)
...
print(f"Top {int(top_ratio*100)}% of grids covered {coverage_rate*100:.2f}% of existing locations.")
# 6.4. Evaluation Classification
if coverage_rate >= 0.6:
    ...
    print("Predictive Strength: Qualified")
else:
